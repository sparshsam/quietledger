...
// Dynamic — render depends on auth state and localStorage data
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (stored === "dark" || (!stored && window.matchMedia("(prefers-color-scheme: dark)").matches)) {
      queueMicrotask(() => setDarkMode(true));
  ...
  async function handleCsvFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setImportParsedInfo({ count: parsed.rows.length, fileName: file.name });
      setImportNotice(`${parsed.rows.length} rows parsed from "${file.name}".`);
    } catch (err) {
  ...
  if (!hydrated) {
    return (
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", color: "var(--text-tertiary)", fontSize: 14 }}>
        Loading…
      </div>
    );
  }
      ...
      <nav className="navbar" aria-label="Navigation">
        ...
        <div className="navbar-actions">
          <button
            ...
            title="Search ledger"
          >
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" color="var(--text)"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          </button>
          <button
            className="navbar-search-btn"
            onClick={toggleTheme}
          ...
          >
            {darkMode ? (
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" color="var(--text)"><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>
            ) : (
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" color="var(--text)"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
            )}
          </button>
        </div>
      </nav>
