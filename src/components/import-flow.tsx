...
export function ImportFlow({ accounts, transactions, learnings, onImport, onRecordLearning, onClose }: ImportFlowProps) {
  ...
  function transitionTo(next: Step) {
    setAnimating(true);
    setTimeout(() => { setStep(next); setAnimating(false); }, 120);
  }
    ...
    <div className="sheet-overlay" onClick={onClose}>
      <div className="import-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="import-progress">
          {STEP_ORDER.map((s, i) => (
            <div key={s} className={"import-progress-step" + (i <= stepIdx ? " completed" : "") + (i === stepIdx ? " active" : "") + (i === STEP_ORDER.length - 1 ? " last" : "")}>
              ...
              <span className="import-progress-label">{["Choose Account", "Upload Statement", "Review & Import"][i]}</span>
        ...
        <div className={"import-body" + (animating ? " fade" : "")}>
            ...
            <div className="import-step">
              ...
              <div className="import-field">
                ...
                <Select
                  ...
                  options={accounts.filter((a) => !a.archivedAt).map((a) => ({ value: a.id, label: a.name }))}
              ...
              <div className="import-info">
                ...
                <p>• Categories are detected automatically</p>
              </div>

              <div className="import-footer">
                <button className="import-btn-primary" onClick={() => transitionTo("upload")} disabled={!selectedAccountId}>
                  Continue
          ...
          {step === "upload" && (
            <div className="import-step">
              <p className="import-step-desc">
                Drop your bank export file below. We accept CSV and TSV formats.
              </p>
          ...
          {step === "review" && parsedCsv && (
            <div className="import-step">
              <p className="import-step-desc">
                {notice || `${preview.length} transactions detected from "${csvFileName}". Review the categories below.`}
              </p>

              <div className="import-review-table">
                {preview.slice(0, 50).map((row, i) => (
                  <div key={i} className="import-review-row">
                    <span className="import-review-date">{row.transaction?.date ?? "—"}</span>
                    <span className="import-review-desc">{row.transaction?.description ?? "—"}</span>
                    <Select
                    ...
                    />
                    <span className={"import-review-amt" + ((row.transaction?.amount ?? 0) < 0 ? " negative" : " positive")}>
                      {row.transaction ? currency.format(row.transaction.amount) : "—"}
                    </span>
                  </div>
                ))}
                {preview.length > 50 && (
                  <p className="import-review-more">… and {preview.length - 50} more transactions</p>
                )}
              </div>

              <div className="import-footer">
                <button className="import-btn-primary" onClick={handleImport} disabled={validRows.length === 0}>
                  Import {validRows.length} transaction{validRows.length !== 1 ? "s" : ""}
                </button>
                <button className="import-btn-ghost" onClick={() => transitionTo("upload")}>Back</button>
              </div>
            </div>
          )}
        </div>
